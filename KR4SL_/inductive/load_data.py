...
class DataLoader:
    def __init__(self, task_dir, opts):
        ...
        with open("F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\data\inductive\SL\Gene_set.txt") as f:
            self.gene_set = []
            for line in f:
                gtype = line.strip()
                self.gene_set.append(gtype)

        with open("F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\data\inductive\SL\entities.txt") as f:
            self.entity2id = dict()
        ...
        with open("F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\data\inductive\SL\\relations.txt") as f:
        ...
        with open("F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\data\inductive\SL_ind\entities.txt") as f:
        ...
        all_entities = []
        with open('F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\data\\all_entities.txt') as f:
            for line in f:
                all_entities.append(line)
        all_entities = np.array(all_entities)
        bert_path = 'F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\\all_entities_pretrain_emb.npy'
        print(bert_path)
        all_entities_pretrain_emb_org = np.load(bert_path, allow_pickle=True)
        # PCA dimension reduction
        from sklearn.decomposition import PCA
        pca = PCA(n_components=self.hidden_dim)
        pca.fit(all_entities_pretrain_emb_org)
        all_entities_pretrain_emb = pca.transform(
            all_entities_pretrain_emb_org)
        sort_idx = all_entities.argsort()
        entity_trans_idx = sort_idx[np.searchsorted(all_entities, np.array(
            [k for k in self.entity2id.keys()]), sorter=sort_idx)]
        entity_ind_idx = sort_idx[np.searchsorted(all_entities, np.array(
            [k for k in self.entity2id_ind.keys()]), sorter=sort_idx)]
        ...
        self.tra_train = self.read_triples_sl(
            self.trans_dir, 'train.txt', mode='transductive', inv=True)
        self.ind_train = self.read_triples_sl(
            self.ind_dir, 'train.txt', mode='inductive', inv=True)
        ...
        # ind data for test
        self.ind_KG, self.ind_sub = self.load_graph(
            self.ind_train, mode='inductive')

        if not os.path.exists(os.path.join(self.trans_dir, 'valid_filtered.txt')):
            print('filtering out triples without answering genes...')
            self.tra_valid0 = self.read_triples_sl(
                self.trans_dir, 'valid.txt', mode='transductive', inv=True)
            self.tra_test0 = self.read_triples_sl(
                self.trans_dir, 'test.txt', mode='transductive', inv=True)
            self.tra_valid = self.delete_triples(
                self.tra_valid0, mode='transductive')
            self.tra_test = self.delete_triples(
                self.tra_test0, mode='transductive')
            self.save_filtered_triples(
                self.trans_dir, 'valid', self.tra_valid, mode='transductive')
            self.save_filtered_triples(
                self.trans_dir, 'test', self.tra_test, mode='transductive')
            print('# triples filtered out in train: {}'.format(
                len(self.tra_valid0) - len(self.tra_valid)))
            print('# triples filtered out in valid: {}'.format(
                len(self.tra_test0) - len(self.tra_test)))
        else:
            self.tra_valid = self.read_triples_sl(
                self.trans_dir, 'valid_filtered.txt', mode='transductive')
            self.tra_test = self.read_triples_sl(
                self.trans_dir, 'test_filtered.txt', mode='transductive')
        if not os.path.exists(os.path.join(self.ind_dir, 'valid_filtered.txt')):
            print('filtering out triples without answering genes...')
            self.ind_valid0 = self.read_triples_sl(
                self.ind_dir, 'valid.txt', mode='inductive', inv=True)
            self.ind_test0 = self.read_triples_sl(
                self.ind_dir, 'test.txt',  mode='inductive', inv=True)
            self.ind_valid = self.delete_triples(
                self.ind_valid0, mode='inductive')
            self.ind_test = self.delete_triples(
                self.ind_test0, mode='inductive')
            self.save_filtered_triples(
                self.ind_dir, 'valid', self.ind_valid, mode='inductive')
            self.save_filtered_triples(
                self.ind_dir, 'test', self.ind_test, mode='inductive')
            print('# triples filtered out in test: {}'.format(len(
                self.ind_valid0)+len(self.ind_test0) - len(self.ind_valid)-len(self.ind_test)))
        else:
            self.ind_valid = self.read_triples_sl(
                self.ind_dir, 'valid_filtered.txt', mode='inductive')
            self.ind_test = self.read_triples_sl(
                self.ind_dir, 'test_filtered.txt', mode='inductive')
        ...
        self.test_q,  self.test_a = self.ind_val_qry + \
            self.ind_tst_qry, self.ind_val_ans+self.ind_tst_ans
        ...
        config_str = 'n_train(triples):{}, n_valid(answers):{}, n_test(answers):{}'.format(
            self.n_train, self.n_valid, self.n_test)
        print(config_str)
        with open(opts.perf_file, 'a+') as f:
            f.write(config_str+'\n')
        # 在 DataLoader.__init__ 末尾加：
        with open('kg_entity2id.json', 'w') as f:
            json.dump(self.entity2id, f)          # transductive
        with open('kg_relation2id.json', 'w') as f:
            json.dump(self.relation2id, f)
        np.savetxt('kg_train_triples.txt', self.tra_train,
                   fmt='%d', delimiter='\t')
        np.savetxt('kg_valid_triples.txt', self.tra_valid,
                   fmt='%d', delimiter='\t')
    ...
    def read_triples_sl(self, directory, filename, mode='transductive', inv=False):
        ...
        '''
        triples = []
        genes = list(self.entitypeid2geneid.keys()) if mode == 'transductive' else list(
            self.entitypeid2geneid_ind.keys())
    ...
    def load_graph(self, triples, mode='transductive'):
        ...
        n_ent = n_ent_org
        idd = np.concatenate([np.expand_dims(
            entity, 1), idd_rel_id*np.ones((n_ent, 1)), np.expand_dims(entity, 1)], 1)
        KG = np.concatenate([KG, idd], 0)
        n_fact = KG.shape[0]
        M_sub = csr_matrix((np.ones((n_fact,)), (np.arange(
            n_fact), KG[:, 0])), shape=(n_fact, n_ent_org))
        return KG, M_sub
    ...
    def get_neighbors(self, nodes, emb, mode='transductive', last_flag=False, layer_type=False, layer_node_type=None):
        ...
        node_1hot = csr_matrix(
            (np.ones(len(nodes)), (nodes[:, 1], nodes[:, 0])), shape=(n_ent, nodes.shape[0]))
        ...
        if last_flag:
            sampled_edges = sampled_edges[np.isin(
                sampled_edges[:, -1], gene_idx)]
        ...
        if last_flag:
            hidden, h0 = emb
            hidden = hidden[np.isin(
                nodes[:, 1], head_nodes[:, 1].cpu().numpy()), :]
