...
class RED_GNN_trans(torch.nn.Module):
    def __init__(self, params, loader):
        ...
        for i in range(self.n_layer):
            self.gnn_layers.append(
                GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, self.n_rel, act=act))
        self.gnn_layers = nn.ModuleList(self.gnn_layers)

        # self.dropout = nn.Dropout(params.dropout)
        self.W_final = nn.Linear(
            self.hidden_dim, 1, bias=False)         # get score
    ...
    def forward(self, subs, rels, mode='train', explain=False, train_mode='train'):
        ...
        nodes = torch.cat(
            [torch.arange(n).unsqueeze(1).cuda(), q_sub.unsqueeze(1)], 1)

        h0 = torch.zeros((1, n, self.hidden_dim)).cuda()
        hidden = torch.zeros(n, self.hidden_dim).cuda()
        entity_pretrain_emb = torch.tensor(
            entity_pretrain_emb, dtype=torch.float32)
        ...
        for i in range(self.n_layer):
            if i != self.n_layer-1:
                nodes, edges, old_nodes_new_idx, _ = self.loader.get_neighbors(nodes.data.cpu(
                ).numpy(), None, mode=mode, last_flag=False)  # nodes: neighbors of source nodes
            else:
                nodes, edges, old_nodes_new_idx, (hidden, h0) = self.loader.get_neighbors(nodes.data.cpu(
                ).numpy(), (hidden, h0), mode=mode, last_flag=True)  # nodes: neighbors of source nodes
            entity_pretrain_emb = entity_pretrain_emb.to(self.device)
            h_sub = entity_pretrain_emb[edges[:, 1]].cuda(
            ) + entity_pretrain_emb[edges[:, 3]].cuda()
            hidden, alpha = self.gnn_layers[i](q_sub, hidden, edges, nodes.size(
                0), old_nodes_new_idx, entity_pretrain_emb, h_sub)
            h0 = torch.zeros(1, nodes.size(0), hidden.size(
                1)).cuda().index_copy_(1, old_nodes_new_idx, h0)
        ...
        # non_visited entities have scores of zeros
    ...
    def loss(self, scores, labels):
        ...
        base_loss = torch.sum(-pos_scores + max_n.squeeze() +
                              torch.log(torch.sum(torch.exp(scores_gene - max_n), 1)))
        return base_loss
