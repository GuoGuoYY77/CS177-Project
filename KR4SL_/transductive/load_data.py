...
class DataLoader:
    def __init__(self, task_dir, opts):
        ...
        with open("F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\data\\transductive\Gene_set.txt") as f:
            self.gene_set = []
            for line in f:
                etype = line.strip()
                self.gene_set.append(etype)

        with open("F:\Courses\大三下\生物信息学\SL_Project_Baseline\KR4SL\data\\transductive\entities.txt") as f:
            self.entity2id = dict()
        ...
        if not os.path.exists(os.path.join(self.task_dir, 'train_filtered.txt')):
            ...
            self.train_data = self.delete_triples(
                self.train_data0, mode='train')
            self.valid_data = self.delete_triples(
                self.valid_data0, mode='train')
            ...
            print('# triples filtered out in train: {}'.format(
                len(self.train_data0) - len(self.train_data)))
            print('# triples filtered out in valid: {}'.format(
                len(self.valid_data0) - len(self.valid_data)))
            print('# triples filtered out in test: {}'.format(
                len(self.test_data0) - len(self.test_data)))
        else:
            self.train_data = self.read_triples_sl(
                'train_filtered.txt')  # _kgwsl_pathway
        ...
        np.savetxt('kg_train_triples.txt', self.train_data,
                   fmt='%d', delimiter='\t')
        np.savetxt('kg_valid_triples.txt', self.valid_data,
                   fmt='%d', delimiter='\t')
    ...
    def load_test_graph(self, triples):
        KG = np.array(triples)
        idd_rel_id = KG[:, 1].max() + 1
        idd = np.concatenate([np.expand_dims(np.arange(self.n_ent), 1), idd_rel_id *
    ...
    def get_neighbors(self, nodes, emb, mode='train', last_flag=False):
        ...
        if last_flag:
            sampled_edges = sampled_edges[np.isin(
                sampled_edges[:, -1], list(self.entitypeid2geneid.keys()))]
