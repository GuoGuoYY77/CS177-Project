...
class KG4SL(object):
    ...
    def _build_model(self, n_entity, n_relation):
        self.entity_emb_matrix = tf.get_variable(
            ...
            initializer=KG4SL.get_initializer(), 
    ...
    def eval(self, sess, feed_dict, threshold=0.5):
        ...
        scores_binary = (scores >= threshold).astype(np.float32)
        auc = roc_auc_score(y_true=labels, y_score=scores)
        p, r, t = precision_recall_curve(y_true=labels, probas_pred=scores)
        aupr = m.auc(r, p)
        f1 = f1_score(y_true=labels, y_pred=scores_binary)
        
        return nodea_emb, nodeb_emb, scores, scores_binary, auc, f1, aupr

    def get_predictions(self, sess, feed_dict):
        """Get raw predictions and labels for threshold tuning"""
        labels, scores = sess.run([self.labels, self.scores_normalized], feed_dict)
